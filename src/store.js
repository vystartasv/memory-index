...
export class MemoryStore {
    ...
    allChunks() {
        const rows = this.db.prepare('SELECT path, start_line as startLine, end_line as endLine, heading, content, lexical_blob as lexicalBlob, embedding_json as embeddingJson FROM chunks').all();
        return rows.map((row) => ({ ...row, embedding: JSON.parse(row.embeddingJson) }));
    }
    lexicalCandidates(queryTokens, limit = 80) {
